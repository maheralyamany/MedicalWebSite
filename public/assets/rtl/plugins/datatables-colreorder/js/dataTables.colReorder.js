 ...
 * @copyleft   Copyleft 2010-2019 SpryMedia Ltd.
 *
...
var ColReorder = function( dt, opts )
{
	...
	this.s = {
		...
		/**
		 * Number of columns to fix counting from left (not allow to be reordered)
		 *  @property fixedLeft
		 *  @type     int
		 *  @default  0
		 */
...
$.extend( ColReorder.prototype, {
	...
	"_fnConstruct": function ()
	{
		...
		if ( this.s.init.iFixedColumnsRight )
		{
			this.s.fixed = this.s.init.iFixedColumnsRight;
		}

		/* Columns discounted from reordering - counting left to right */
		this.s.fixedLeft = this.s.init.iFixedColumnsLeft ?
		...
		for ( i = 0; i < iLen; i++ )
		{
			if ( i > this.s.fixed-1 && i < iLen - this.s.fixedLeft )
			{
		...
		if ( aiOrder )
		{
			 ...
			 * to wait until the draw is done, if after, then do what we need to do left away
			 */
	...
	"_fnMouseDown": function ( e, nTh )
	{
		...
		this.s.mouse.offsetX = this._fnCursorPosition( e, 'pageX' ) - offset.right;
	...
	"_fnMouseMove": function ( e )
	{
		...
		this.dom.drag.css( {
			right: this._fnCursorPosition( e, 'pageX' ) - this.s.mouse.offsetX,
        ...
        if (target) {
            this.dom.pointer.css('right', target.x);
            this.s.mouse.toIndex = target.to;
        }
        else {
			// The insert element wasn't positioned in the array (less than
			// operator), so we put it at the end
			this.dom.pointer.css( 'right', lastNotHidden().x );
	...
	"_fnRegions": function ()
	{
        ...
        for (var i = 0; i < aoColumnBounds.length; i++) {
            ...
            * position is just to it's immediate right or left, so we only increment the counter for
            * other columns
            */
		...
		if ( this.s.fixedLeft !== 0 )
		{
			this.s.aoTargets.splice( this.s.aoTargets.length - this.s.fixedLeft );
		}
...
ColReorder.defaults = {
	 ...
	 * right). This will typically be 1 if used, but can be as high as you like.
	 *  @type int
	 *  @default 0
	 *  @static
	 */
	...
	/**
	 * As `iFixedColumnsLeft` but counting from the left.
	 *  @type int
	 *  @default 0
	 *  @static
	 */
