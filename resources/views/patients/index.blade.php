            ...
            <tbody>
                @if (isset($patients) && $patients->count() > 0)
                    @foreach ($patients as $patient)
                        <tr>
                            <td>
                                <img src="{{ $patient->getPatientPhoto() }}" alt="maxSoft"
                ...
                @else
                    <tr class="odd">
                        <td valign="top" colspan="7" class="dataTables_empty">{{ trans('m.filter_dataTables_empty') }}
                        </td>
                    </tr>
                @endif
            </tbody>
    ...
    <script>
        $(function() {
             dynamicDataTable('#dynamic-table', {
                ...
                paging: true,
                oLanguage:{
                    sSearchPlaceholder: "{{trans('m.search_patients')}}",
                }
            });
